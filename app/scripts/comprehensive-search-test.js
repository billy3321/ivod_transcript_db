...
async function analyzeCurrentData() {
  ...
  try {
    ...
    logError(`資料庫分析失敗: ${error.message}`);
    return false;
  } finally {
    await prisma.$disconnect();
  }
...
async function main() {
  console.log('');
  log('🔍 IVOD Transcript DB - 全面搜尋功能測試', 'bright');
  console.log('');
  ...
  try {
    // 分析當前資料狀況
    const hasData = await analyzeCurrentData();
    if (!hasData) {
      return;
    }
    ...
    logSection('測試總結');
    logSuccess('全面搜尋功能測試完成');
    
    logInfo('\n測試涵蓋的使用場景：');
    logInfo('1. ✅ 基本搜尋功能 (上方搜尋欄位)');
    logInfo('2. ✅ 搜尋模式切換 (所有欄位 vs 僅搜尋逐字稿)');
    logInfo('3. ✅ 進階篩選功能 (下方進階選項)');
    logInfo('4. ✅ 混合搜尋 (上方搜尋 + 下方篩選)');
    logInfo('5. ✅ 進階搜尋語法 (引號、布林運算等)');
    logInfo('6. ✅ 分頁和排序');
    logInfo('7. ✅ URL 編碼處理');
    logInfo('8. ✅ 邊界情況和錯誤處理');
    
    logInfo('\n如果發現問題：');
    logInfo('1. 檢查資料庫中的測試資料是否充足');
    logInfo('2. 確認委員會名稱需要完整匹配');
    logInfo('3. 驗證進階搜尋語法的解析');
    logInfo('4. 檢查 URL 編碼/解碼的處理');
